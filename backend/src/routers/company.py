...
async def get_all_companies(
    *,
    ...
    _: Annotated[Auth, Depends(require_permission(Permission.READ_COMPANY))],
    sort: SortQuery,
...
async def create_a_company(
    service: Annotated[CompanyService, Depends()],
    _: Annotated[Auth, Depends(require_permission(Permission.CREATE_COMPANY))],
...
async def update_a_company(
    service: Annotated[CompanyService, Depends()],
    _: Annotated[Auth, Depends(require_permission(Permission.UPDATE_COMPANY))],
...
async def delete_a_company(
    service: Annotated[CompanyService, Depends()],
    _: Annotated[Auth, Depends(require_permission(Permission.DELETE_COMPANY))],
    identifier: Annotated[int, Path()],
) -> None:
